...
impl<T> FetchDirect<T> {
    ...
    pub fn new(vec:Vec<T>) -> Self {
        ...
        let optimal_q_size = vec.len() / max_threads / 2;                    
        Self {
            vec:RawVec::new(vec),
...
impl<T> DiscreteQueue for FetchDirect<T> {
    ...
    //     let ctr = self.ctr.fetch_add(1,Ordering::AcqRel);                 
    //     if ctr >= self.vec.len() {
    //         let _ = self.active.compare_exchange(true, false, Ordering::Release, Ordering::Relaxed);
    //         return None;
    //     }                       
    //     self.vec.take(ctr)              
    // }
